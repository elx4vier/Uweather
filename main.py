...
def convert_temperature(temp, unit="c"):
    return int(temp * 9/5 + 32) if unit.lower() == "f" else int(temp)
...
class UWeather(Extension):
    ...
    def on_preferences_changed(self, key, value):
        ...
        if key in ["provider", "api_key", "unit", "location_mode"]:
            ...
            ThreadPoolExecutor(max_workers=1).submit(self.update_location)
        elif key == "static_location" and (self.preferences.get("location_mode") or "auto") == "manual":
            provider = (self.preferences.get("provider") or "open-meteo").lower()
    ...
    def update_location(self):
        """
        Atualiza localização automática ou manual e salva no cache.
        """
...
class WeatherListener(EventListener):
    def on_event(self, event, extension):
        ...
        if not query:
            ...
            return self.search_city_weather(query, extension, unit, provider, interface)
    ...
    def search_city_weather(self, query, extension, unit, provider, interface):
        city_query, country_filter = query, None
        if "," in city_query:
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                for res in results[:3]:
                    lat, lon = res.get("latitude") or res.get("lat"), res.get("longitude") or res.get("lon")
                    if lat is None or lon is None: continue
                    future_to_geo[executor.submit(extension.get_weather_data, lat, lon, unit)] = res
    ...
    def render(self, cached_item, extension, interface_mode, return_item=False):
        ...
        if interface_mode == "complete":
            ...
            desc_text = f"Amanhã: {forecast[0]['min']}º/{forecast[0]['max']}º" if forecast else "Clique para detalhes"
